...
void print_estado()
{
	printf("Estado: 0 => %s\t1 => %s\t2 => %s\t3 => %s\t4 => %s\n",
	        estado(estado_filosofo[0]), estado(estado_filosofo[1]), estado(estado_filosofo[2]),
	        estado(estado_filosofo[3]), estado(estado_filosofo[4]));
}
...
void *filosofo(void *num) {
	...
	while (1) {
		...
		palillo[fil_id] = 0; //desocupas los dos palillos
		palillo[(fil_id+1)%FILOSOFOS] = 0; //desocupas los dos palillos
...
int main(void) {
	...
	for(i = 0; i < FILOSOFOS; i++) {
		palillo[i] = 0; //estan todos libres
